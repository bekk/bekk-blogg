...
export const Header = ({ isInArticle = false }: HeaderProps) => {
  ...
  return (
    <div
      className={`flex items-end justify-between pb-8 md:pb-14 ${isInArticle ? 'pt-6 px-2 md:pl-16 md:pt-5' : 'pt-4 md:px-10 md:pt-8'}`}
    >
      {!error && (
      ...
      <div className={`shrink-0 px-4 ${error ? 'ml-auto' : ''}`}>
        <Link to="/">
          {isInArticle ? (
            <PostStamp />
          ) : (
