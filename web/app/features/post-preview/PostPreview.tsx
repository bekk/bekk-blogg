...
export const PostPreview = ({
  ...
  const content = (
    <div className="grid sm:grid-cols-[1fr_1px_1fr] grid-cols-[30fr_1fr] w-full py-6 px-6 sm:p-7">
  ...
  return <div className="striped-frame max-w-4xl">{link ? <Link to={link}>{content}</Link> : content}</div>
}
    ...
    <div className="flex flex-col max-sm:w-full gap-8 md:gap-12">
      {posts.map((post) => (
        <div key={post._id} className="flex justify-center">
          <PostPreview {...post} link={postUrl(post)} />
        </div>
      ))}
    </div>
  )
