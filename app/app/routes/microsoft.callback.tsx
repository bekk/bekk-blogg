...
const logIntoSanity = async (token: ExtendedJwtPayload): Promise<string> => {
  try {
    const existingSanityUser = await getSanityUser()
    console.info(`User ${token.email} ${existingSanityUser ? 'is already' : 'was not'} logged into Sanity`)
    ...
    if (existingSanityUser?.provider === 'external') {
      console.info(`User ${token.email} had a valid session in Sanity, redirecting to the Studio`)
...
async function createOrUpdateAuthorInSanity(token: ExtendedJwtPayload) {
  ...
  if (existingAuthor) {
    console.info(`Found an existing author called ${token.name}. Updating it with newest data`)
  } else {
    console.info(`Could not find a matching author for ${token.name}. Creating it.`)
  }
  const authorId = existingAuthor ? existingAuthor._id : userIdFromEmail(token.email)
...
async function addUserToGroup(sanityAuthorId: string) {
  ...
  if (!members.includes(userId)) {
    ...
    return authClient.patch(group._id).setIfMissing({ members: [] }).append('members', [userId]).commit()
  }
}
...
async function getEndUserClaimUrl(sanityUser: SanityDocument, token: ExtendedJwtPayload) {
